...
function makePulseRequests(type){
    ...
    https.get(allUrls[i], function(res){
      ...
      res.on('end', function(){
        ...
        if(count==5){
          ...
          }else if(type == 'trainAndTest'){
            var singleDataArray = createSingleDataArray(allData);
            var testData = [];
            var obj = {};
            for(var j=0; j<singleDataArray.length; j++){
              obj = {};
              for(var key in singleDataArray[j]){
                if(key == 'power')
                  obj[key]='--'
                else
                  obj[key] = singleDataArray[j][key];
              }
              testData.push(obj);
            }
            applyMachineLearning({'train': singleDataArray, 'test': testData}, 'trainAndTest');
          }
        }
      });
    }).on('error', function(e){
...
exports.trainAndTest = function(){
  return makePulseRequests('trainAndTest');
}
...
function applyMachineLearning(data, type){
  ...
  for(var i=0; i<data.length; i++){
    ...
    input.date = type == 'csv' ? (new Date(data[i].date).getTime()/1000)%secondsInDay/secondsInDay : (data[i].seconds % secondsInDay) / secondsInDay;
  ...
  if(type == "csv" || type == 'trainAndTest'){
    ...
    for(var i = 0; i<64; i++){
      ...
      datePlus15.setMinutes(datePlus15.getMinutes() + ((i)*15));
      dateSeconds = datePlus15.getTime()/1000;
      forecastData.push({date: (dateSeconds % secondsInDay)/secondsInDay});
      secondsToTime[(dateSeconds % secondsInDay)/secondsInDay] = formatLocalDate(datePlus15);
    }
  }
  ...
  for(var i = 0; i<forecastData.length; i++){
    ...
    finalDataArray.push({
      "date": type == "csv" || type == 'trainAndTest' ? futureData[i].date : secondsToTime[forecastData[i].date], 
      "radiation": type == "csv" || type == 'trainAndTest' ? (!isNaN(futureData[i].radiation) ? futureData[i].radiation : '--') : '--',
      "humidity": type == "csv" || type == 'trainAndTest' ? (!isNaN(futureData[i].humidity) ? futureData[i].humidity : '--') : '--',
      "temperature": type == "csv" || type == 'trainAndTest' ? (!isNaN(futureData[i].temperature) ? futureData[i].temperature : '--') : '--',
  ...
  if(type == "pulse"){
    ...
    for(var j=0; j<finalDataArray.length; j++){
      //We're going to be plotting actual power vs that forecasted by the 
      //learning algorithm. On the x axis we have the date and on the y axis
      //we have the power.
      finalDataArray[j]['x'] = new Date(finalDataArray[j]['date']).getTime();
      finalDataArray[j]['y'] = finalDataArray[j]['power'];

      if(finalDataArray[j]['forecast'] && finalDataArray[j]['forecast'] == true){
        testData.push(finalDataArray[j]);
      }else{
        trainData.push(finalDataArray[j]);
      }

    }
    server.mlPerformanceData({'test':testData, 'train':trainData});
  }
